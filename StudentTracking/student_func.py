...
def center_window(self, w, h): # passes in the tkinter frame (master) reference and the w and h (from main)
...
def ask_quit(self):
    if messagebox.askokcancel("Exit program", "Okay to exit application?"): # first string is box title, second is message
        # To close app:
        self.master.destroy()
        os._exit(0) # ensures clears memory when close
...
def first_run(self):
    data = ('John', 'Doe', 'John Doe', '111-111-1111', 'jdoe@gmail.com', 'Intro to Python')
    conn = sqlite3.connect('db_phonebook.db')
    with conn:
        cur = conn.cursor()
        cur,count = count_records(cur)
        if count < 1:
            cur.execute("""INSERT INTO tbl_phonebook (col_fname,col_lname,col_fullname,col_phone,col_email,col_course) VALUES (?,?,?,?,?,?)""", ('John','Doe','John Doe','111-111-1111','jdoe@email.com','Intro to Python'))
            conn.commit()
    conn.close()
...
# Function for selecting item in ListBox
def onSelect(self,event):
    # calling the event is the self.lstList1 widget
    varList = event.widget
    select = varList.curselection()[0] # this will only yield the index position
    value = varList.get(select) # this saves the value in the index position as 'value'
    conn = sqlite3.connect('db_phonebook.db')
    with conn:
        cursor = conn.cursor()
...
def addToList(self):
    ...
    if (len(var_fname) > 0) and (len(var_lname) > 0) and (len(var_phone) > 0) and (len(var_email) > 0) and (len(var_course) > 0): # enforce the user to provide both
        conn = sqlite3.connect('db_phonebook.db')
        with conn:
            cursor = conn.cursor()
...
def onDelete(self):
    var_select = self.lstList1.get(self.lstList1.curselection()) # Listbox's selected value by user, stored 
    conn = sqlite3.connect('db_phonebook.db')
    with conn:
        cur = conn.cursor()
